...
const reportTypes = [
  ...
  { id: "inventory", name: "Inventaire", icon: FileText, color: "border-violet-200 bg-violet-50 text-violet-700" },
]
...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-2xl">
      <div><h1 className="text-2xl font-bold text-gray-800">Rapports</h1><p className="text-sm text-gray-500">Générez des rapports dans différents formats</p></div>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
        ...
        <button onClick={handleGenerate} disabled={loading} className="w-full py-3 text-white rounded-xl font-medium bg-gradient-to-r from-emerald-500 to-teal-600 hover:shadow-lg disabled:opacity-50">
          <Download size={18} className="inline mr-2" />{loading ? "Génération..." : "Générer le rapport"}
        </button>
      </div>
    </div>
  )
